...
import type { ReelProductionManifest, ReelProductionStatus, ReelRenderedOutput, WordTiming } from "./types";
...
export const reelProductionService = {
  ...
  async attachShotAsset(input: {
    ...
    for (const dependencyId of shot.dependsOnShotIds) {
      ...
      if (!dependency?.asset?.videoUrl || !["GENERATED", "PASSED"].includes(dependency.status)) {
        throw new Error(`Shot ${input.shotId} dependency ${dependencyId} is not generated`);
      }
    }
  ...
  async attachNarratedRoughCut(input: {
    id: string;
    output: ReelRenderedOutput;
    expectedRevision?: number;
  }) {
    const stored = await reelProductionStore.get(input.id);
    if (!stored) throw new Error(`Production ${input.id} not found`);
    ...
    if (Math.abs(input.output.actualDurationSec - stored.manifest.plannedDurationSec) > 0.25) throw new Error("Narrated rough-cut duration does not match the canonical timeline");
