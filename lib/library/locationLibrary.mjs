...
function getPostgresPool() {
  ...
  if (!pgPool) {
    try {
      ...
      console.warn("[locationLibrary.mjs] Failed to initialize PostgreSQL pool:", e.message);
      return null;
    }
  }
  return pgPool;
...
export const locationLibrary = {
  ...
  async get(id) {
    ...
    const db = getDatabase();
    const row = db.prepare("SELECT * FROM location_library WHERE id = ?").get(id);
    if (!row) return null;
      ...
      try {
        await pg.query(`
          ...
          ON CONFLICT (id) DO UPDATE SET
            ...
            environment_block = EXCLUDED.environment_block,
