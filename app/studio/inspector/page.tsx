...
import { ArrowLeft, Download, Film, Music2, Mic2, ScanSearch } from "lucide-react";
...
export default function ReelInspectorPage() {
  ...
  useEffect(() => { fetch("/api/reels/productions?limit=25", { cache: "no-store" }).then(r => r.json()).then(d => { const p = d.productions || []; setProductions(p); if (p[0]?.id) setProductionId(p[0].id); }).catch(e => setError(String(e))); }, []);
  useEffect(() => { if (!productionId) return; setInspection(null); setFrame(0); fetch(`/api/reels/productions/${encodeURIComponent(productionId)}/inspection`, { cache: "no-store" }).then(r => r.json()).then(d => { if (!d.success) throw new Error(d.error); setInspection(d.inspection); }).catch(e => setError(e?.message || String(e))); }, [productionId]);
    ...
    <main className="mx-auto max-w-[1600px] p-6">
      ...
      {!inspection ? <div className="rounded-2xl border border-white/10 p-8 text-slate-500">Loading inspection evidence…</div> : <div className="grid gap-5 xl:grid-cols-[430px_1fr]">
        ...
        <section className="space-y-4">
          ...
          <Panel title="Frame truth"><Rows data={{ frame: current?.frame, timeSec: current?.timeSec, shotId: current?.shotId, shotFrame: current?.shotFrame, boundary: current?.boundary?.strategy || null }}/></Panel>
