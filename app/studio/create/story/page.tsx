...
<package xmlns="http://www.idpf.org/2007/opf" unique-identifier="zyvoriq-myth-${Date.now()}" version="3.0">
  ...
  <manifest>
    ...
    ${generatedVideoUrl ? `<item id="audio_reel" href="${generatedVideoUrl}" media-type="video/mp4"/>` : ""}
  </manifest>
  <spine>
    <itemref idref="cover"/>
    <itemref idref="act1"/>
  ...
  const handleDownloadCueSheet = () => {
    ...
    const cueSheet = {
      ...
      ambience,
      illumination,
      masterVideoReel: generatedVideoUrl || null,
...
================================================================================`;
  ...
  const handleDownloadVideo = () => {
    if (!generatedVideoUrl) {
      alert("No synthesized video available. Please synthesize the Heritage & Mythology reel first.");
      return;
    }
    setIsDownloading(true);
    const a = document.createElement("a");
            ...
            <div className="lg:col-span-7 space-y-6">
              ...
              {activeTab === "video" && (
                <div className="space-y-5">
                  <div className="relative rounded-3xl border border-amber-500/30 bg-black overflow-hidden shadow-2xl aspect-[9/16] max-h-[640px] mx-auto flex flex-col items-center justify-center p-6 text-center">
                    {generatedVideoUrl ? (
                      <video
                      ...
                      <div className="flex flex-col items-center justify-center space-y-4">
                        ...
                        <div className="inline-flex items-center gap-2 rounded-full border border-amber-500/30 bg-amber-500/10 px-3.5 py-1 text-xs font-mono font-bold text-amber-300">
                          HERITAGE DIFFUSION STAGE · AWAITING SYNTHESIS
                        </div>
                        <h4 className="text-base font-serif font-bold text-white max-w-xs">
                          Antiquity & Vedic Lore Synthesizer
                        </h4>
                        <p className="text-xs text-slate-400 max-w-xs font-mono">
                          Zero fallback media. Click 'Synthesize Mythology Reel' to render original 4K cinematic lore with orchestral audio.
                        </p>
                        <button
                          type="button"
                          onClick={handleGenerate}
                          className="mt-2 px-5 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-black text-xs transition cursor-pointer"
                        >
                          🔱 Synthesize Mythology Reel
                        </button>
                      </div>
                    )}
