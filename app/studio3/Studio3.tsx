...
type ClipOption = { id: string; label: string; asset: { videoUrl: string; actualDurationSec: number; model?: string }; createdAt: string };
...
type Studio3Meta = {
  ...
  clipOptions: Record<string, ClipOption[]>;
  omniModel: string;
  omniOptions: Record<string, OmniOption[]>;
...
export function Studio3() {
  ...
  async function refresh(id: string) {
    const response = await fetch(`/api/studio3/productions/${encodeURIComponent(id)}`, { cache: "no-store" });
    const data = await response.json();
    if (!response.ok || !data.success) throw new Error(data.error || "Failed to refresh Studio3");
  ...
  async function waitForOperation(operationId: string, productionId: string) {
    ...
    while (Date.now() < deadline) {
      ...
      const op = data.operation as DurableOperation;
      if (op.status === "SUCCEEDED") return refresh(productionId);
      if (op.status === "FAILED") throw new Error(op.lastError || "Generation failed");
      await sleep(1500);
    }
  ...
  async function dispatch(current: StoredProduction, action: string, extra: Record<string, unknown> = {}) {
    const response = await fetch(`/api/studio3/productions/${encodeURIComponent(current.id)}`, {
      method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, expectedRevision: current.revision, ...extra }),
    });
    const data = await response.json();
  ...
  async function buildPlan() {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok || !data.success) throw new Error(data.error || "Failed to build Studio3 plan");
      setProduction(data.production); setSelectedShotId(null);
    } catch (e: any) { setError(e?.message || "Failed to build plan"); } finally { setBusy(null); }
  }
  async function generateAll(fresh = false) {
    if (!production) return;
    setBusy(fresh ? "fresh" : "all"); setError(""); setSelectedShotId(null);
    try {
  ...
  async function rebuildFullReel() {
    if (!production) return;
    setBusy("rough"); setError("");
    try { const current = await dispatch(production, "renderNarratedRoughCut"); setProduction(current); setSelectedShotId(null); }
    catch (e: any) { setError(e?.message || "Full reel rebuild failed"); }
    finally { setBusy(null); }
  }
  async function regenerate(shotId: string) {
    if (!production) return;
    setBusy(`regen:${shotId}`); setError("");
    try { setProduction(await dispatch(production, "regenerateShot", { shotId, modelTier: "fast" })); setSelectedShotId(shotId); }
    catch (e: any) { setError(e?.message || "Regeneration failed"); }
    finally { setBusy(null); }
  }
  async function setContinuity(enabled: boolean) {
    if (!production) return;
    setError(""); try { setProduction(await dispatch(production, "setPresenterContinuity", { enabled })); } catch (e: any) { setError(e?.message || "Failed to update presenter continuity"); }
  }
  async function setEnvironmentContinuity(enabled: boolean) {
    if (!production) return;
    setError(""); try { setProduction(await dispatch(production, "setEnvironmentContinuity", { enabled })); } catch (e: any) { setError(e?.message || "Failed to update environment continuity"); }
  }
  async function setSubjectMode(shotId: string, mode: SubjectMode) {
    if (!production) return;
    setError(""); try { setProduction(await dispatch(production, "setSubjectMode", { shotId, mode })); } catch (e: any) { setError(e?.message || "Failed to update subject mode"); }
  }
  async function selectClipOption(shotId: string, optionId: string) {
    if (!production) return;
    setError(""); try { setProduction(await dispatch(production, "selectOption", { shotId, optionId })); setSelectedShotId(shotId); } catch (e: any) { setError(e?.message || "Failed to select clip option"); }
  }
  async function omni(mode: OmniMode) {
    if (!production || !selectedShotId) return;
    setBusy(`omni:${mode}`); setError("");
    try {
      ...
      const data = await response.json();
      if (!response.ok || !data.success) throw new Error(data.error || `Omni ${mode.toLowerCase()} failed`);
      setProduction(data.production);
  ...
  async function selectOmni(shotId: string, optionId?: string) {
    if (!production) return;
    setError(""); try { setProduction(await dispatch(production, "selectOmniOption", { shotId, optionId })); setSelectedShotId(shotId); } catch (e: any) { setError(e?.message || "Failed to select Omni option"); }
  }
  ...
  return <div className="min-h-screen bg-[#07090d] text-slate-100">
    ...
    <main className="mx-auto grid max-w-[1600px] gap-5 px-5 py-6 md:px-8 lg:grid-cols-[350px_1fr_380px]">
      <aside className="h-fit rounded-[26px] border border-white/10 bg-white/[0.025] p-5 lg:sticky lg:top-24">
        ...
        {manifest && <div className="mt-4 space-y-3">
          <label className="flex items-center justify-between rounded-2xl border border-cyan-300/15 bg-cyan-300/[0.04] p-3"><div><div className="flex items-center gap-2 text-sm font-black"><UserRoundCheck className="h-4 w-4 text-cyan-300" />Presenter continuity</div><div className="mt-1 text-[11px] text-slate-500">Same canonical presenter in every PRESENTER clip.</div></div><input type="checkbox" checked={Boolean(meta?.presenterContinuity)} onChange={e => setContinuity(e.target.checked)} className="h-5 w-5 accent-cyan-300" /></label>
          <label className="flex items-center justify-between rounded-2xl border border-cyan-300/15 bg-cyan-300/[0.04] p-3"><div><div className="text-sm font-black">Environment continuity</div><div className="mt-1 text-[11px] text-slate-500">Preserve location, background, lighting and major props.</div></div><input type="checkbox" checked={meta?.environmentContinuity !== false} onChange={e => setEnvironmentContinuity(e.target.checked)} className="h-5 w-5 accent-cyan-300" /></label>
        </div>}
      ...
      <section className="rounded-[26px] border border-white/10 bg-[#0a0d12] p-6">
        ...
        {manifest && <div className="mt-7 space-y-4">{shots.map((shot, index) => {
          const omniOptions = meta?.omniOptions?.[shot.id] || [];
          const clipOptions = meta?.clipOptions?.[shot.id] || [];
          const selected = meta?.selectedOmniOptionIds?.[shot.id];
          const subjectMode = meta?.subjectModes?.[shot.id] || "PRESENTER";
          return <div key={shot.id} className={`rounded-2xl border p-4 ${selectedShotId === shot.id ? "border-violet-300/35 bg-violet-300/[0.04]" : "border-white/10"}`}>
            <div className="flex gap-3"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white/5 text-xs font-black">{index + 1}</div><div className="min-w-0 flex-1"><div className="text-sm leading-6 text-slate-300">{shot.visualIntent}</div><div className="mt-1 text-xs text-slate-600">{shot.asset?.videoUrl ? `${shot.asset.actualDurationSec?.toFixed(2) || "?"}s · ${shot.asset.model || "Veo"}` : "Pending"}</div>
              <div className="mt-3 flex flex-wrap gap-2"><select value={subjectMode} onChange={e => setSubjectMode(shot.id, e.target.value as SubjectMode)} disabled={Boolean(busy)} className="rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-xs font-bold"><option value="PRESENTER">PRESENTER · canonical identity</option><option value="NO_PERSON">NO PERSON · strict B-roll</option></select>{shot.asset?.videoUrl && <button onClick={() => { setSelectedShotId(shot.id); selectOmni(shot.id, undefined); }} className="rounded-xl border border-white/10 px-3 py-2 text-xs font-black">Original</button>}{shot.asset?.videoUrl && <button onClick={() => setSelectedShotId(shot.id)} className="rounded-xl border border-violet-300/20 bg-violet-300/[0.05] px-3 py-2 text-xs font-black text-violet-100"><WandSparkles className="mr-1 inline h-3.5 w-3.5" />Omni tools</button>}{manifest.audio?.narrationUrl && <button onClick={() => regenerate(shot.id)} disabled={Boolean(busy)} className="rounded-xl border border-white/10 px-3 py-2 text-xs font-black disabled:opacity-50">Regenerate Veo</button>}</div>
              {clipOptions.length > 0 && <div className="mt-3 rounded-xl border border-white/5 bg-black/15 p-3"><div className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-600">Previous immutable Veo options</div><div className="mt-2 flex flex-wrap gap-2">{clipOptions.map(option => <button key={option.id} onClick={() => selectClipOption(shot.id, option.id)} disabled={Boolean(busy)} className="rounded-lg border border-white/10 px-2.5 py-1.5 text-[11px] font-bold text-slate-400 hover:text-white">{option.label} · {option.asset.actualDurationSec?.toFixed(2) || "?"}s</button>)}</div></div>}
              {omniOptions.length > 0 && <div className="mt-3 rounded-xl border border-violet-300/10 bg-violet-300/[0.025] p-3"><div className="text-[10px] font-black uppercase tracking-[0.14em] text-violet-300/70">Omni review options</div><div className="mt-2 flex flex-wrap gap-2">{omniOptions.map(option => <button key={option.id} onClick={() => selectOmni(shot.id, option.id)} className={`rounded-lg border px-2.5 py-1.5 text-[11px] font-bold ${selected === option.id ? "border-violet-300/40 text-violet-100" : "border-white/10 text-slate-400"}`}>Omni {option.mode} · expires {new Date(option.expiresAt).toLocaleDateString()}</button>)}</div></div>}
            </div></div>
          </div>;
        })}</div>}
      ...
      <aside className="h-fit lg:sticky lg:top-24">
        ...
        {selectedShot && <div className="mt-4 rounded-[24px] border border-violet-300/20 bg-violet-300/[0.035] p-5"><div className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.14em] text-violet-300"><Scissors className="h-4 w-4" /> Gemini Omni 1.1 Flash</div><p className="mt-2 text-xs leading-5 text-slate-500">Edit or repair the selected generated clip without rerunning Veo. Extension appends to the end and follows Omni uploaded-video constraints.</p><textarea value={omniPrompt} onChange={e => setOmniPrompt(e.target.value)} rows={5} className="mt-4 w-full rounded-xl border border-white/10 bg-black/20 p-3 text-sm outline-none" />
          ...
          <div className="mt-4 rounded-xl border border-amber-300/15 bg-amber-300/[0.04] p-3 text-[11px] leading-5 text-amber-100/70">Omni outputs use Gemini Files URI delivery and expire after about 48 hours. They remain review options and never overwrite the durable Veo source used by the current full-reel renderer.</div>
        </div>}
        {manifest && <div className="mt-4 rounded-[24px] border border-white/10 p-5 text-sm"><div className="flex items-center gap-2 font-black"><UserRoundCheck className="h-4 w-4 text-cyan-300" />Studio3 truth</div><div className="mt-3 space-y-2 text-xs text-slate-500"><div>Production: {production?.id}</div><div>Presenter continuity: {meta?.presenterContinuity ? "ON" : "OFF"}</div><div>Environment continuity: {meta?.environmentContinuity ? "ON" : "OFF"}</div><div>Omni: {meta?.omniModel || "—"}</div><div>Combined MP4: {roughCut ? "Ready" : "Pending"}</div></div></div>}
      </aside>
    </main>
  </div>;
}
