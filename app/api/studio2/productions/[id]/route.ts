...
async function enqueueShot(production: NonNullable<Awaited<ReturnType<typeof studio2Service.get>>>, shotId: string, modelTier: "fast" | "quality" | "lite" = "fast", nonce?: string) {
  const shot = production.manifest.shots.find(item => item.id === shotId);
  if (!shot) throw new Error(`Shot ${shotId} not found`);
  const control = await paidContext(production.id);
  const fp = fingerprint({ prompt: shot.generationPrompt, duration: shot.generationDurationSec, modelTier, round: (production.manifest as any).studio2?.generationRound, nonce });
  ...
  return reelOperationQueue.enqueue({ productionId: production.id, kind: "SHOT", targetId: shot.id, idempotencyKey, payload: { manifestRevision: production.revision, generationToken: control.generationToken, semanticFingerprint: fp, modelTier, studio2: true } });
}
...
export async function GET(_req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ success: false, error: error?.message || "Failed to load Studio2 production" }, { status: 500 });
  }
}
...
export async function PATCH(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await context.params;
    if (!id.startsWith("studio2_")) return NextResponse.json({ success: false, error: "Studio2 may only mutate studio2_-prefixed productions" }, { status: 403 });
    const body = await req.json();
    const action = String(body.action || "");
    const expectedRevision = body.expectedRevision === undefined ? undefined : Number(body.expectedRevision);
    let current = await studio2Service.get(id);
    if (!current) return NextResponse.json({ success: false, error: "Studio2 production not found" }, { status: 404 });
    ...
    if (action === "setPresenterContinuity") {
      const production = await studio2Service.setPresenterContinuity(id, Boolean(body.enabled), current.revision);
    ...
    if (action === "selectOption") {
      const production = await studio2Service.selectOption(id, String(body.shotId || ""), String(body.optionId || ""), current.revision);
      return NextResponse.json({ success: true, production });
    }

    if (action === "generateAllFresh") {
      const production = await studio2Service.prepareFreshAll(id, current.revision);
      return NextResponse.json({ success: true, production, fresh: true });
    }

    if (action === "regenerateShot") {
      current = await studio2Service.prepareShotRegeneration(id, String(body.shotId || ""), current.revision);
    ...
    if (action === "generateNarration") {
      ...
      const fp = fingerprint({ script: current.manifest.masterScript, tone: current.manifest.tone, studio2: true });
      ...
      const operation = await reelOperationQueue.enqueue({ productionId: id, kind: "NARRATION", idempotencyKey, payload: { manifestRevision: current.revision, generationToken: control.generationToken, semanticFingerprint: fp, studio2: true } });
      return NextResponse.json({ success: true, queued: true, operation, production: current }, { status: 202 });
    }
    ...
    if (action === "renderNarratedRoughCut") {
      ...
      const control = await paidContext(id);
      const fp = fingerprint({ audio: current.manifest.audio.narrationUrl, shots: current.manifest.shots.map(shot => [shot.id, shot.asset?.videoUrl, shot.trimInSec, shot.trimOutSec]), studio2: true });
      ...
      const operation = await reelOperationQueue.enqueue({ productionId: id, kind: "ROUGH_CUT", idempotencyKey, payload: { manifestRevision: current.revision, generationToken: control.generationToken, semanticFingerprint: fp, studio2: true } });
      return NextResponse.json({ success: true, queued: true, operation, production: current }, { status: 202 });
    }

    return NextResponse.json({ success: false, error: `Unsupported Studio2 action: ${action}` }, { status: 400 });
  } catch (error: any) {
