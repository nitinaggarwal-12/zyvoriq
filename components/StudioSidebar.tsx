...
export function StudioSidebar({ children, currentPath }: StudioSidebarProps) {
  ...
  return (
    <div className="min-h-dvh flex flex-col md:flex-row bg-obsidian-950 text-slate-100 selection:bg-teal-500/30 w-full overflow-x-hidden pb-20 md:pb-0">
      ...
      >
        {/* NATIVE MOBILE / DESKTOP APP BAR */}
        <header className="sticky top-0 z-30 flex h-14 w-full items-center justify-between border-b border-slate-800/80 bg-obsidian-950/85 px-4 sm:px-6 backdrop-blur-xl">
          {/* Left: Mobile App Brand Icon / Title */}
          <div className="flex items-center gap-2.5 min-w-0">
            <Link href="/" className="flex md:hidden items-center gap-2">
              <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-br from-teal-400 to-emerald-500 font-mono text-xs font-black text-obsidian-950">
                Z
              </div>
            </Link>
            <div className="flex items-center gap-1.5 min-w-0">
              <span className="text-xs font-bold text-white font-mono uppercase tracking-tight truncate">
                {currentSectionTitle()}
              </span>
              <span className="hidden sm:inline-block h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse" />
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => setApiKeyModalOpen(true)}
              className="flex items-center gap-1 px-2.5 py-1 rounded-lg border border-amber-500/30 bg-amber-500/10 text-[10px] font-mono font-bold text-amber-300"
            >
            ...
            <Link
              href="/director"
              className="flex items-center gap-1 rounded-xl bg-gradient-to-r from-teal-400 to-emerald-500 px-3 py-1.5 text-xs font-black text-slate-950 shadow-md shadow-teal-500/10 hover:opacity-90 active:scale-95 transition"
            >
