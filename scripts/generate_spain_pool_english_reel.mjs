...
async function main() {
  ...
  execSync(`ffmpeg -y -ss 0.350 -t 5.250 -i "${shot1Mp4}" -vf "${videoPolishWide}" -af "aresample=48000,asetpts=PTS-STARTPTS" -r 30 -ar 48000 -ac 2 -c:v libx264 -preset fast -crf 16 -c:a aac -b:a 256k "${s1}"`, { stdio: 'pipe' });
  execSync(`ffmpeg -y -ss 0.000 -t 5.250 -i "${shot2Mp4}" -vf "${videoPolishMcu}" -af "aresample=48000,asetpts=PTS-STARTPTS" -r 30 -ar 48000 -ac 2 -c:v libx264 -preset fast -crf 16 -c:a aac -b:a 256k "${s2}"`, { stdio: 'pipe' });
  execSync(`ffmpeg -y -ss 0.000 -t 5.000 -i "${shot3Mp4}" -vf "${videoPolishWide}" -af "aresample=48000,asetpts=PTS-STARTPTS" -r 30 -ar 48000 -ac 2 -c:v libx264 -preset fast -crf 16 -c:a aac -b:a 256k "${s3}"`, { stdio: 'pipe' });
  ...
  execSync(
    `ffmpeg -y -i "${unifiedBaseMp4}" -vn -af "equalizer=f=60:width_type=o:width=1.5:g=2.5,equalizer=f=3200:width_type=o:width=1.5:g=2.0,acompressor=threshold=-16dB:ratio=2.2:attack=15:release=120,loudnorm=I=-14:TP=-1.0:LRA=11,aresample=48000" -ar 48000 -ac 2 -c:a aac -b:a 256k "${audioMasterM4a}"`,
    { stdio: 'pipe' }
  );

  const sub1Png = await createSubtitlePng('♪ "Under the Spanish sun we shine so bright, dancing in the pool till the Ibiza night!" ♪', path.join(SCRATCH_DIR, 'sub1.png'));
