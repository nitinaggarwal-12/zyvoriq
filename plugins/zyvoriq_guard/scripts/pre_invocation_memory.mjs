  ...
  try {
    ...
    if (workspace.includes("zyvoriq")) {
      const reminder = [
        ...
        "13. MANDATORY SEQUENTIAL TAIL-FRAME CHAINING (ZERO ANCHOR RE-USE RESET): In Veo image-to-video diffusion, never pass the same anchor image to consecutive shots (Shot 1, Shot 2, Shot 3). Shot N+1 MUST condition on the extracted tail frame of Shot N (ffmpeg -sseof -0.1 ...). Initial-frame PSNR across cut boundaries must be < 25 dB to guarantee forward motion without snap-back loops.",
        "14. AUDIO-VISUAL VOCAL COINCIDENCE & ZERO INSTRUMENTAL MOUTHING: Pre-flight acoustic analysis must extract vocal onset T_vocal. During instrumental intros (0s - T_vocal), video prompts must mandate pure dance choreography and fashion modeling with mouth closed. Characters must never mouth lyrics or articulate words during instrumental music."
      ].join("\n");
