...
## 9. 🛡️ `governance-doc-lockstep-sync` (Universal Post-Fix Governance Synchronization — v5.2.0)
...
2. **Automated Lockstep Verification**:
   - Run `node scripts/guards/gate_governance_doc_sync.mjs` to assert that all canonical files share the exact same version badge (`v5.2.0`) and never fall out of sync.
